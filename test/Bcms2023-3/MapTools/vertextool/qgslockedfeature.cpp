...
QgsLockedFeature::QgsLockedFeature(QgsFeatureId featureId,
                                   QgsVectorLayer *layer, QgsMapCanvas *canvas)
    : mFeatureId(featureId), mLayer(layer), mCanvas(canvas) {
    replaceVertexMap();
}
...
void QgsLockedFeature::updateGeometry(const QgsGeometry *geom) {
    ...
    if (!geom) {
        QgsFeature f;
        mLayer->getFeatures(QgsFeatureRequest().setFilterFid(mFeatureId))
            .nextFeature(f);
        if (f.hasGeometry())
...
void QgsLockedFeature::beforeRollBack() {
    disconnect(mLayer, &QgsVectorLayer::geometryChanged, this,
               &QgsLockedFeature::geometryChanged);
    deleteVertexMap();
}
...
void QgsLockedFeature::endGeometryChange() {
    ...
    connect(mLayer, &QgsVectorLayer::geometryChanged, this,
            &QgsLockedFeature::geometryChanged);
}

void QgsLockedFeature::featureDeleted(QgsFeatureId fid) {
    if (fid == mFeatureId) deleteLater();
}

void QgsLockedFeature::geometryChanged(QgsFeatureId fid,
                                       const QgsGeometry &geom) {
    QgsDebugCall;

    if (!mLayer || fid != mFeatureId) return;
...
void QgsLockedFeature::validateGeometry(QgsGeometry *g) {
    ...
    connect(mValidator, &QgsGeometryValidator::errorFound, this,
            &QgsLockedFeature::addError);
    connect(mValidator, &QThread::finished, this,
            &QgsLockedFeature::validationFinished);
    mValidator->start();

    // QgsStatusBar *sb = BcmsApp::instance()->statusBarIface();
    // sb->showMessage(tr("Validation started."));
    auto *mb = BcmsApp::instance()->messageBar();
    mb->pushMessage(tr("Validation started."), tr(""),
                    Qgis::MessageLevel::Info);
}

void QgsLockedFeature::addError(QgsGeometry::Error e) {
    mGeomErrors << e;
    if (!mTip.isEmpty()) mTip += '\n';
    mTip += e.what();

    if (e.hasWhere()) {
        QgsVertexMarker *marker = new QgsVertexMarker(mCanvas);
        marker->setCenter(
            mCanvas->mapSettings().layerToMapCoordinates(mLayer, e.where()));
...
void QgsLockedFeature::validationFinished() {
    ...
    //                    mGeomErrorMarkers.size()));

    auto *mb = BcmsApp::instance()->messageBar();
    mb->pushMessage(tr("Validation finished (%n error(s) found).",
                       "number of geometry errors", mGeomErrorMarkers.size()),
                    tr(""), Qgis::MessageLevel::Warning);
}
...
void QgsLockedFeature::selectVertex(int vertexNr) {
    if (vertexNr < 0 || vertexNr >= mVertexMap.size()) return;
...
void QgsLockedFeature::invertVertexSelection(
    const QVector<int> &vertexIndices) {
    const auto constVertexIndices = vertexIndices;
    for (const int index : constVertexIndices) {
        if (index < 0 || index >= mVertexMap.size()) continue;
...
QgsFeatureId QgsLockedFeature::featureId() { return mFeatureId; }

QList<QgsVertexEntry *> &QgsLockedFeature::vertexMap() { return mVertexMap; }

QgsVectorLayer *QgsLockedFeature::layer() { return mLayer; }
