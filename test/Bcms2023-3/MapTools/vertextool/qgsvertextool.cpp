...
//! Find out whether vertex at the given index is an endpoint (assuming linear
//! geometry)
static bool isEndpointAtVertexIndex(const QgsGeometry &geom, int vertexIndex) {
    ...
    if (const QgsCurve *curve = qgsgeometry_cast<const QgsCurve *>(g)) {
    ...
    } else if (const QgsMultiCurve *multiCurve =
                   qgsgeometry_cast<const QgsMultiCurve *>(g)) {
        ...
        QgsDebugMsg(QStringLiteral(
            "is_endpoint_at_vertex_index: unexpected geometry type!"));
        return false;
    }
}

//! Returns the index of vertex adjacent to the given endpoint. Assuming linear
//! geometries.
int adjacentVertexIndexToEndpoint(const QgsGeometry &geom, int vertexIndex) {
    ...
    if (const QgsCurve *curve = qgsgeometry_cast<const QgsCurve *>(g)) {
        ...
        for (int i = 0; i < multiCurve->numGeometries(); ++i) {
            ...
            Q_ASSERT(part);
            if (vertexIndex < part->numPoints())
                return vertexIndex == 0 ? offset + 1
                                        : offset + part->numPoints() - 2;
            vertexIndex -= part->numPoints();
            offset += part->numPoints();
        }
    } else {
        QgsDebugMsg(QStringLiteral(
            "adjacent_vertex_index_to_endpoint: unexpected geometry type!"));
    }
    return -1;
}
...
static bool isCircularVertex(const QgsGeometry &geom, int vertexIndex) {
    QgsVertexId vid;
    return geom.vertexIdFromVertexNr(vertexIndex, vid) &&
           vid.type == Qgis::VertexType::Curve;
}

//! Create a multi-point geometry that can be used to highlight vertices of a
//! feature
static QgsGeometry geometryToMultiPoint(const QgsGeometry &geom) {
    ...
    for (auto pointIt = geom.vertices_begin(); pointIt != geom.vertices_end();
         ++pointIt)
...
class MatchCollectingFilter : public QgsPointLocator::MatchFilter {
   public:
    ...
    MatchCollectingFilter(QgsVertexTool *vertextool) : vertextool(vertextool) {}

    bool acceptMatch(const QgsPointLocator::Match &match) override {
        if (match.distance() > 0) return false;

        // there may be multiple points at the same location, but we get only
        // one result... the locator API needs a new method verticesInRect()
        QgsGeometry matchGeom =
            vertextool->cachedGeometry(match.layer(), match.featureId());
        ...
        while (matchGeom.constGet()->nextVertex(vid, pt)) {
            ...
            if (pt.x() == match.point().x() && pt.y() == match.point().y()) {
                if (isPolygon) {
                    // for polygons we need to handle the case where the first
                    // vertex is matching because the last point will have the
                    // same coordinates and we would have a duplicate match
                    // which would make subsequent code behave incorrectly
                    // (topology editing mode would add a single vertex twice)
                    if (vid.vertex == 0) {
                        polygonRingVid = vid;
                    } else if (vid.ringEqual(polygonRingVid) &&
                               vid.vertex == matchGeom.constGet()->vertexCount(
                                                 vid.part, vid.ring) -
                                                 1) {
                        continue;
                    }
                ...
                QgsPointLocator::Match extra_match(match.type(), match.layer(),
                                                   match.featureId(), 0,
                                                   match.point(), vindex);
...
/**
 * Keeps the best match from a selected feature so that we can possibly use it
 * with higher priority. If we do not encounter any selected feature within
 * tolerance, we use the best match as usual.
 *
 * There are two kinds of "selected" features... if we have a "locked" feature
 * in vertex tool, we get a non-null QgsLockedFeature which has feature's layer
 * and feature id. In such case we only allow matches from that feature. The
 * other kind is if some features within layer are in the layer's list of
 * selected features - in that case we accept any match from those features (but
 * only if we do not have "locked" feature)
 */
class SelectedMatchFilter : public QgsPointLocator::MatchFilter {
   public:
    explicit SelectedMatchFilter(double tol, QgsLockedFeature *selectedFeature)
        : mTolerance(tol), mLockedFeature(selectedFeature) {}

    bool acceptMatch(const QgsPointLocator::Match &match) override {
        if (match.distance() <= mTolerance && match.layer()) {
            // option 1: we have "locked" feature - we consider just a match
            // from that feature option 2: we do not have "locked" feature - we
            // consider matches from any selected feature
            if ((mLockedFeature && mLockedFeature->layer() == match.layer() &&
                 mLockedFeature->featureId() == match.featureId()) ||
                (!mLockedFeature &&
                 match.layer()->selectedFeatureIds().contains(
                     match.featureId()))) {
                if (!mBestSelectedMatch.isValid() ||
                    match.distance() < mBestSelectedMatch.distance())
    ...
    QgsPointLocator::Match bestSelectedMatch() const {
        return mBestSelectedMatch;
    }

   private:
    double mTolerance;
    QgsLockedFeature
        *mLockedFeature;  // not null in case of selected (locked) feature
...
QgsVertexTool::QgsVertexTool(QgsMapCanvas *canvas,
...
void QgsVertexTool::activate() {
    if (BcmsApp::instance() &&
        QgsVertexEditor::settingAutoPopupVertexEditorDock.value()) {
        showVertexEditor();  // #spellok
    }

    if (QgsVertexEditor *editor = vertexEditor()) {
        connect(editor, &QgsVertexEditor::deleteSelectedRequested, this,
                &QgsVertexTool::deleteVertexEditorSelection);
        connect(editor, &QgsVertexEditor::editorClosed, this,
                &QgsVertexTool::cleanupVertexEditor);
    }

    connect(mCanvas, &QgsMapCanvas::currentLayerChanged, this,
            &QgsVertexTool::currentLayerChanged);
...
void QgsVertexTool::deactivate() {
    ...
    cleanupVertexEditor();
    if (QgsVertexEditor *editor = vertexEditor()) {
        disconnect(editor, &QgsVertexEditor::deleteSelectedRequested, this,
                   &QgsVertexTool::deleteVertexEditorSelection);
        disconnect(editor, &QgsVertexEditor::editorClosed, this,
                   &QgsVertexTool::cleanupVertexEditor);
    }

    mSnapIndicator->setMatch(QgsPointLocator::Match());

    QHash<QPair<QgsVectorLayer *, QgsFeatureId>, GeometryValidation>::iterator
        it = mValidations.begin();
    for (; it != mValidations.end(); ++it) it->cleanup();
    mValidations.clear();

    disconnect(mCanvas, &QgsMapCanvas::currentLayerChanged, this,
               &QgsVertexTool::currentLayerChanged);
...
void QgsVertexTool::addDragStraightBand(QgsVectorLayer *layer, QgsPointXY v0,
                                        QgsPointXY v1, bool moving0,
                                        bool moving1,
                                        const QgsPointXY &mapPoint) {
    // if layer is not null, the input coordinates are coming in the layer's CRS
    // rather than map CRS
    if (layer) {
...
void QgsVertexTool::addDragCircularBand(QgsVectorLayer *layer, QgsPointXY v0,
                                        QgsPointXY v1, QgsPointXY v2,
                                        bool moving0, bool moving1,
                                        bool moving2,
                                        const QgsPointXY &mapPoint) {
    // if layer is not null, the input coordinates are coming in the layer's CRS
    // rather than map CRS
...
void QgsVertexTool::cadCanvasPressEvent(QgsMapMouseEvent *e) {
    ...
    if (!mDraggingVertex && !mSelectedVertices.isEmpty() &&
        ...
        if (m.hasVertex()) {
            for (const Vertex &selectedVertex :
                 std::as_const(mSelectedVertices)) {
                if (selectedVertex.layer == m.layer() &&
                    selectedVertex.fid == m.featureId() &&
                    selectedVertex.vertexId == m.vertexIndex()) {
    ...
    if (e->button() == Qt::LeftButton) {
        if (e->modifiers() & Qt::ControlModifier ||
            e->modifiers() & Qt::ShiftModifier) {
            // shift or ctrl-click vertices to highlight without entering edit
            // mode
            QgsPointLocator::Match m = snapToEditableLayer(e);
            if (m.hasVertex()) {
...
void QgsVertexTool::cadCanvasReleaseEvent(QgsMapMouseEvent *e) {
    ...
    if (mNewVertexFromDoubleClick) {
        ...
        if (mLockedFeature && (mLockedFeature->featureId() != m.featureId() ||
                               mLockedFeature->layer() != m.layer()))
            return;  // when a feature is bound to the vector editor, only
                     // process actions on that feature

        mNewVertexFromDoubleClick.reset();

        // dragging of edges and double-clicking on edges to add vertex are
        // slightly overlapping so we need to cancel edge moving before we start
        // dragging new vertex
        stopDragging();
    ...
    } else if (mSelectionRubberBand && (mSelectionMethod == SelectionNormal ||
        ...
        // - if we have a bound (locked) feature, we only allow selection of its
        // vertices
        // - if we don't have a bound feature, we can select vertices from any
        // feature,
        //   but if there are some vertices coming from layer(s) with selected
        //   feature, we give them precedence.
        ...
        for (QgsVectorLayer *vlayer : editableLayers) {
            ...
            if (mLockedFeature && mLockedFeature->layer() != vlayer)
                continue;  // with locked feature we only allow selection of its
                           // vertices

            QgsGeometry layerRubberBandGeometry = rubberBandGeometry;
            try {
                QgsCoordinateTransform ct =
                    mCanvas->mapSettings().layerTransform(vlayer);
                if (ct.isValid())
                    layerRubberBandGeometry.transform(
                        ct, Qgis::TransformDirection::Reverse);
            } catch (QgsCsException &) {
                continue;
            }

            QgsRenderContext context =
                QgsRenderContext::fromMapSettings(mCanvas->mapSettings());
            ...
            std::unique_ptr<QgsGeometryEngine> layerRubberBandEngine(
                QgsGeometry::createGeometryEngine(
                    layerRubberBandGeometry.constGet()));
            ...
            if (r)
                request.setSubsetOfAttributes(r->usedAttributes(context),
                                              vlayer->fields());
            else
            ...
            while (fi.nextFeature(f)) {
                if (mLockedFeature && mLockedFeature->featureId() != f.id())
                    continue;  // with locked feature we only allow selection of
                               // its vertices

                context.expressionContext().setFeature(f);
                bool isFeatureInvisible =
                    (r && !r->willRenderFeature(f, context));
                // If we've already determined that we have to show users a
                // warning about selecting invisible vertices, then we don't
                // need to check through the vertices for other invisible
                // features. We'll be showing the warning anyway regardless of
                // the outcome!
                if (isFeatureInvisible && showInvisibleFeatureWarning) continue;

                bool isFeatureSelected =
                    vlayer->selectedFeatureIds().contains(f.id());
                QgsGeometry g = f.geometry();
                int i = 0;
                for (auto it = g.constGet()->vertices_begin();
                     it != g.constGet()->vertices_end(); ++it) {
                    QgsPoint pt = *it;
                    if (layerRubberBandEngine->contains(&pt)) {
                        // we don't want to select invisible features,
                        // but if the user tried to selected one we want to warn
                        // him
            ...
            if (r) r->stopRender(context);
        }
        if (showInvisibleFeatureWarning) {
            BcmsApp::instance()->messageBar()->pushMessage(
                tr("Invisible vertices were not selected"),
                tr("Vertices belonging to features that are not displayed on "
                   "the map canvas were not selected."),
                Qgis::MessageLevel::Warning);
        }

        // here's where we give precedence to vertices of selected features in
        // case there's no bound (locked) feature
    ...
    } else  // selection rect is not being dragged
    {
        if (e->button() == Qt::LeftButton &&
            ...
            if (mDraggingVertex || mDraggingEdge) {
                ...
                // Right-click to select/delect a feature for editing (also gets
                // selected in vertex editor). If there are multiple features at
                // one location, cycle through them with subsequent right
                // clicks.
...
void QgsVertexTool::cadCanvasMoveEvent(QgsMapMouseEvent *e) {
    if (mLockedFeatureAlternatives &&
        (e->pos() - mLockedFeatureAlternatives->screenPoint)
                .manhattanLength() >= QApplication::startDragDistance()) {
        // as soon as the mouse moves more than just a tiny bit, previously
        // stored alternatives info is probably not valid anymore and will need
        // to be re-calculated
    ...
    if (mDraggingVertex) {
        ...
        if (!mSelectionRubberBand &&
            (e->pos() - *mSelectionRubberBandStartPos).manhattanLength() >=
                10) {
...
void QgsVertexTool::moveDragBands(const QgsPointXY &mapPoint) {
    for (int i = 0; i < mDragStraightBands.count(); ++i) {
        ...
        if (b.moving0) b.band->movePoint(0, mapPoint + b.offset0);
        if (b.moving1) b.band->movePoint(1, mapPoint + b.offset1);
    }
...
void QgsVertexTool::mouseMoveDraggingEdge(QgsMapMouseEvent *e) {
    ...
    QgsPointXY mapPoint = toMapCoordinates(
        e->pos());  // do not use e.mapPoint() as it may be snapped
...
QgsPointLocator::Match QgsVertexTool::snapToEditableLayer(QgsMapMouseEvent *e) {
    ...
    typedef QHash<QgsVectorLayer *, QgsSnappingConfig::IndividualLayerSettings>
        SettingsHashMap;
    ...
    // and selecting current layer is an easy way for the user to prioritize a
    // layer
    if (QgsVectorLayer *currentVlayer = currentVectorLayer()) {
        if (currentVlayer->isEditable()) {
            ...
            for (QgsMapLayer *layer : layers) {
                ...
                SettingsHashMap::const_iterator existingSettings =
                    oldLayerSettings.find(vlayer);
                if (existingSettings != oldLayerSettings.constEnd()) {
                    ...
                    layerSettings.setTypeFlag(static_cast<Qgis::SnappingTypes>(
                    ...
                    layerSettings = QgsSnappingConfig::IndividualLayerSettings(
                        vlayer == currentVlayer,
                        static_cast<Qgis::SnappingTypes>(
                            Qgis::SnappingType::Vertex |
                            Qgis::SnappingType::Segment),
                        tol, QgsTolerance::ProjectUnits, 0.0, 0.0);
                }
    ...
    // if there is no match from the current layer, try to use any editable
    // vector layer
    ...
    if (mLastSnap) {
        ...
        QgsPointLocator::Match lastMatch =
            snapUtils->snapToMap(mapPoint, &filterLast, true);
        // but skip the the previously used feature if it would only snap to
        // segment, while now we have snap to vertex so that if there is a point
        // on a line, it gets priority (as is usual with combined vertex+segment
        // snapping)
        bool matchHasVertexLastHasEdge = m.hasVertex() && lastMatch.hasEdge();
        if (lastMatch.isValid() && lastMatch.distance() <= m.distance() &&
            !matchHasVertexLastHasEdge) {
            m = lastMatch;
        }
    }
...
QgsPointLocator::Match QgsVertexTool::snapToPolygonInterior(
    QgsMapMouseEvent *e) {
    QgsSnappingUtils *snapUtils = canvas()->snappingUtils();
    QgsPointLocator::Match m;
    ...
    if (QgsVectorLayer *currentVlayer = currentVectorLayer()) {
        if (currentVlayer->isEditable() &&
            currentVlayer->geometryType() == QgsWkbTypes::PolygonGeometry) {
            QgsPointLocator::MatchList matchList =
                snapUtils->locatorForLayer(currentVlayer)
                    ->pointInPolygon(mapPoint, true);
    ...
    if (!m.isValid() && mMode == AllLayers) {
        ...
        for (QgsMapLayer *layer : layers) {
            ...
            if (vlayer->isEditable() &&
                ...
                QgsPointLocator::MatchList matchList =
                    snapUtils->locatorForLayer(vlayer)->pointInPolygon(mapPoint,
                                                                       true);
                if (!matchList.isEmpty()) {
                    m = matchList.first();
                    break;
...
QList<QgsPointLocator::Match> QgsVertexTool::findEditableLayerMatches(
    const QgsPointXY &mapPoint, QgsVectorLayer *layer) {
    QgsPointLocator::MatchList matchList;

    if (!layer->isEditable()) return matchList;
    ...
    double tolerance =
        QgsTolerance::vertexSearchRadius(canvas()->mapSettings());
...
QSet<QPair<QgsVectorLayer *, QgsFeatureId>>
QgsVertexTool::findAllEditableFeatures(const QgsPointXY &mapPoint) {
    ...
    // layer
    if (QgsVectorLayer *currentVlayer = currentVectorLayer()) {
        for (const QgsPointLocator::Match &m :
             findEditableLayerMatches(mapPoint, currentVlayer)) {
    ...
    if (mMode == AllLayers) {
        ...
        for (QgsMapLayer *layer : layers) {
            ...
            for (const QgsPointLocator::Match &m :
                 findEditableLayerMatches(mapPoint, vlayer)) {
                alternatives.insert(qMakePair(m.layer(), m.featureId()));
            }
        }
    }
...
void QgsVertexTool::tryToSelectFeature(QgsMapMouseEvent *e) {
    if (!mLockedFeatureAlternatives) {
        // this is the first right-click on this location so we currently do not
        // have information about editable features at this mouse location -
        // let's build the alternatives info
        QSet<QPair<QgsVectorLayer *, QgsFeatureId>> alternatives =
            findAllEditableFeatures(toMapCoordinates(e->pos()));
        if (!alternatives.isEmpty()) {
            QgsPointLocator::Match m = snapToEditableLayer(e);
            if (!m.isValid()) {
                // as the last resort check if we are on top of a feature if
                // there is no vertex or edge snap
            ...
            if (m.isValid()) {
                // ideally the feature that would get normally highlighted
                // should be also the first choice because as user moves mouse,
                // different features are highlighted, so the highlighted
                // feature should be first to get selected
                QPair<QgsVectorLayer *, QgsFeatureId> firstChoice(
                    m.layer(), m.featureId());
            ...
            mLockedFeatureAlternatives->alternatives.append(
                qgis::setToList(alternatives));

            if (mLockedFeature) {
                // in case there is already a feature locked, continue in the
                // loop from it
                QPair<QgsVectorLayer *, QgsFeatureId> currentSelection(
                    mLockedFeature->layer(), mLockedFeature->featureId());
                int currentIndex =
                    mLockedFeatureAlternatives->alternatives.indexOf(
                        currentSelection);
                if (currentIndex != -1)
    ...
    if (mLockedFeatureAlternatives) {
        ...
        if (mLockedFeatureAlternatives->index <
            mLockedFeatureAlternatives->alternatives.count() - 1)
    ...
    if (mLockedFeatureAlternatives && mLockedFeatureAlternatives->index != -1) {
        ...
        QPair<QgsVectorLayer *, QgsFeatureId> alternative =
            mLockedFeatureAlternatives->alternatives.at(
                mLockedFeatureAlternatives->index);
        // keep only corrsesponding vertices
        // todo: it might be nice to keep other vertices in memory, so we could
        // select them when switching lokcked feature
        QList<Vertex> vertices;
        ...
        // there's really nothing under the cursor or while cycling through the
        // list of available features we got to the end of the list - let's
        // deselect any feature we may have had selected
        setHighlightedVertices(QList<Vertex>(), ModeReset);
        cleanupLockedFeature();
    }

    // we have either locked ourselves to a feature or unlocked again
    // in any case, we don't want feature highlight anymore (vertex editor has
    // its own highlight)
    updateFeatureBand(QgsPointLocator::Match());
}

bool QgsVertexTool::isNearEndpointMarker(const QgsPointXY &mapPoint) {
    if (!mEndpointMarkerCenter) return false;
...
QgsPointXY QgsVertexTool::positionForEndpointMarker(
    const QgsPointLocator::Match &match) {
    QgsGeometry geom = cachedGeometry(match.layer(), match.featureId());

    QgsPointXY pt0 =
        geom.vertexAt(adjacentVertexIndexToEndpoint(geom, match.vertexIndex()));
    ...
    double angle = std::atan2(dy, dx);  // to the top: angle=0, to the right:
                                        // angle=90, to the left: angle=-90
...
void QgsVertexTool::mouseMoveNotDragging(QgsMapMouseEvent *e) {
    ...
    bool targetIsAllowed =
        (!mLockedFeature || (mLockedFeature->featureId() == m.featureId() &&
                             mLockedFeature->layer() == m.layer()));
    ...
    if (m.type() == QgsPointLocator::Vertex && targetIsAllowed) {
        ...
        if (isMatchAtEndpoint(m)) {
            mMouseAtEndpoint.reset(
                new Vertex(m.layer(), m.featureId(), m.vertexIndex()));
            mEndpointMarkerCenter.reset(
                new QgsPointXY(positionForEndpointMarker(m)));
    ...
    if (m.type() == QgsPointLocator::Edge && targetIsAllowed) {
        ...
        if (isCircularVertex(geom, m.vertexIndex())) {
            ...
            isCircularEdge = true;
            QgsPointXY pX =
                toMapCoordinates(m.layer(), geom.vertexAt(m.vertexIndex() - 1));
            QgsPointSequence points;
            QgsGeometryUtils::segmentizeArc(QgsPoint(pX), QgsPoint(p0),
                                            QgsPoint(p1), points);
            mEdgeBand->reset();
            ...
            isCircularEdge = true;
            QgsPointXY pX =
                toMapCoordinates(m.layer(), geom.vertexAt(m.vertexIndex() + 2));
            QgsPointSequence points;
            QgsGeometryUtils::segmentizeArc(QgsPoint(p0), QgsPoint(p1),
                                            QgsPoint(pX), points);
            mEdgeBand->reset();
            for (const QgsPoint &pt : std::as_const(points))
                mEdgeBand->addPoint(pt);
            ...
            points << p0 << p1;
            mEdgeBand->setToGeometry(QgsGeometry::fromPolylineXY(points),
                                     nullptr);
        }
        ...
        mEdgeCenterMarker->setVisible(
            !isCircularEdge);  // currently not supported for circular edges
    ...
    if (!m.isValid()) {
        // as the last resort check if we are on top of a feature if there is no
        // vertex or edge snap
        m = snapToPolygonInterior(e);
    }

    // when we are "locked" to a feature, we don't want to highlight any other
    // features so the user does not get distracted
...
void QgsVertexTool::updateVertexBand(const QgsPointLocator::Match &m) {
    if (m.hasVertex() && m.layer()) {
        mVertexBand->setToGeometry(QgsGeometry::fromPointXY(m.point()),
                                   nullptr);
        mVertexBand->setVisible(true);
        bool isCircular = false;
        if (m.layer()) {
            isCircular = isCircularVertex(
                cachedGeometry(m.layer(), m.featureId()), m.vertexIndex());
        }

        mVertexBand->setIcon(isCircular ? QgsRubberBand::ICON_FULL_DIAMOND
...
void QgsVertexTool::updateFeatureBand(const QgsPointLocator::Match &m) {
    ...
    if (m.isValid() && m.layer()) {
        ...
        QgsGeometry geom = cachedGeometry(m.layer(), m.featureId());
        mFeatureBandMarkers->setToGeometry(geometryToMultiPoint(geom),
                                           m.layer());
...
void QgsVertexTool::keyPressEvent(QKeyEvent *e) {
    switch (e->key()) {
        case Qt::Key_Escape: {
            if (mSelectionMethod == SelectionRange) stopRangeVertexSelection();
            if (mSelectionMethod == SelectionPolygon) stopSelectionRubberBand();
            if (mDraggingVertex || mDraggingEdge) stopDragging();
            break;
        }
        case Qt::Key_Delete:
        case Qt::Key_Backspace: {
            ...
            if (mDraggingVertex ||
                (!mDraggingEdge && !mSelectedVertices.isEmpty())) {
                e->ignore();  // Override default shortcut management
                deleteVertex();
            }
            break;
        ...
        case Qt::Key_R: {
            if (e->modifiers() & Qt::ShiftModifier && !mDraggingVertex &&
                !mDraggingEdge)
        ...
        case Qt::Key_Less:
        case Qt::Key_Comma: {
            if (!mDraggingVertex && !mDraggingEdge) highlightAdjacentVertex(-1);
            break;
        }
        case Qt::Key_Greater:
        case Qt::Key_Period: {
...
QgsGeometry QgsVertexTool::cachedGeometry(const QgsVectorLayer *layer,
                                          QgsFeatureId fid) {
    ...
    if (layerWasNotInCache) {
        connect(layer, &QgsVectorLayer::geometryChanged, this,
                &QgsVertexTool::onCachedGeometryChanged);
        connect(layer, &QgsVectorLayer::featureDeleted, this,
                &QgsVertexTool::onCachedGeometryDeleted);
        connect(layer, &QgsVectorLayer::willBeDeleted, this,
                &QgsVertexTool::clearGeometryCache);
        connect(layer, &QgsVectorLayer::dataChanged, this,
                &QgsVertexTool::clearGeometryCache);
    }

    if (!layerCache.contains(fid)) {
        QgsFeature f;
        layer->getFeatures(QgsFeatureRequest(fid).setNoAttributes())
            .nextFeature(f);
...
void QgsVertexTool::clearGeometryCache() {
    const QgsVectorLayer *layer =
        qobject_cast<const QgsVectorLayer *>(sender());
    mCache.remove(layer);
    disconnect(layer, &QgsVectorLayer::geometryChanged, this,
               &QgsVertexTool::onCachedGeometryChanged);
    disconnect(layer, &QgsVectorLayer::featureDeleted, this,
               &QgsVertexTool::onCachedGeometryDeleted);
    disconnect(layer, &QgsVectorLayer::willBeDeleted, this,
               &QgsVertexTool::clearGeometryCache);
    disconnect(layer, &QgsVectorLayer::dataChanged, this,
               &QgsVertexTool::clearGeometryCache);
}

void QgsVertexTool::onCachedGeometryChanged(QgsFeatureId fid,
                                            const QgsGeometry &geom) {
    QgsVectorLayer *layer = qobject_cast<QgsVectorLayer *>(sender());
    Q_ASSERT(mCache.contains(layer));
    QHash<QgsFeatureId, QgsGeometry> &layerCache = mCache[layer];
    if (layerCache.contains(fid)) layerCache[fid] = geom;
    ...
    if (mLockedFeature && mLockedFeature->featureId() == fid &&
        mLockedFeature->layer() == layer) {
...
void QgsVertexTool::onCachedGeometryDeleted(QgsFeatureId fid) {
    ...
    Q_ASSERT(mCache.contains(layer));
    QHash<QgsFeatureId, QgsGeometry> &layerCache = mCache[layer];
    if (layerCache.contains(fid)) layerCache.remove(fid);
...
void QgsVertexTool::updateVertexEditor(QgsVectorLayer *layer,
                                       QgsFeatureId fid) {
    if (layer) {
        if (mLockedFeature && mLockedFeature->featureId() == fid &&
            mLockedFeature->layer() == layer) {
            // if show feature is called on a feature that's already binded to
            // the vertex editor, toggle it off
            cleanupLockedFeature();
            return;
        }

        mLockedFeature.reset(new QgsLockedFeature(fid, layer, mCanvas));
        connect(mLockedFeature->layer(), &QgsVectorLayer::featureDeleted, this,
                &QgsVertexTool::cleanEditor);
        for (int i = 0; i < mSelectedVertices.length(); ++i) {
            if (mSelectedVertices.at(i).layer == layer &&
                mSelectedVertices.at(i).fid == fid) {
                mLockedFeature->selectVertex(mSelectedVertices.at(i).vertexId);
            }
        }
        connect(mLockedFeature.get(), &QgsLockedFeature::selectionChanged, this,
                &QgsVertexTool::lockedFeatureSelectionChanged);
    }
    ...
    if (editor && mLockedFeature) {
        ...
        for (const QgsVertexEntry *vertex : vertexMap) {
            if (!vertex->isSelected()) {
                ...
                marker->setCenter(
                    toMapCoordinates(mLockedFeature->layer(), vertex->point()));
...
QgsVertexEditor *QgsVertexTool::vertexEditor() {
    return nullptr;
    // return BcmsApp::instance() ? BcmsApp::instance()->vertexEditor() : nullptr;
}

void QgsVertexTool::showVertexEditor()  // #spellok
{
    if (QgsVertexEditor *editor = vertexEditor()) editor->setUserVisible(true);
}
...
void QgsVertexTool::lockedFeatureSelectionChanged() {
    ...
    for (int i = 0, n = vertexMap.size(); i < n; ++i) {
        if (vertexMap[i]->isSelected()) {
            vertices << Vertex(mLockedFeature->layer(),
                               mLockedFeature->featureId(), i);
        }
    }
...
void QgsVertexTool::deleteVertexEditorSelection() {
    ...
    if (!mLockedFeature->geometry()->isNull()) {
        ...
        if (mLockedFeature->geometry()->type() == QgsWkbTypes::LineGeometry) {
            // for lines we don't wrap around vertex selection when deleting
            // vertices from end of line
            nextVertexToSelect = std::min(
                nextVertexToSelect,
                mLockedFeature->geometry()->constGet()->nCoordinates() - 1);
        }
...
void QgsVertexTool::startDragging(QgsMapMouseEvent *e) {
    ...
    if (mLockedFeature && (mLockedFeature->featureId() != m.featureId() ||
                           mLockedFeature->layer() != m.layer()))
        return;  // when a feature is bound to the vertex editor, only process
                 // actions for that feature
...
QSet<Vertex> QgsVertexTool::findCoincidentVertices(
    const QSet<Vertex> &vertices) {
    ...
    for (const Vertex &v : std::as_const(vertices)) {
        ...
        for (QgsVectorLayer *vlayer : editableLayers) {
            const auto snappedVertices =
                layerVerticesSnappedToPoint(vlayer, mapPointV);
            for (const QgsPointLocator::Match &otherMatch : snappedVertices) {
                Vertex otherVertex(otherMatch.layer(), otherMatch.featureId(),
                                   otherMatch.vertexIndex());
...
void QgsVertexTool::buildExtraVertices(const QSet<Vertex> &vertices,
                                       const QgsPointXY &anchorPoint,
                                       QgsVectorLayer *anchorLayer) {
    for (const Vertex &v : std::as_const(vertices)) {
        if (mDraggingVertex && v == *mDraggingVertex) continue;

        QgsPointXY pointV = cachedGeometryForVertex(v).vertexAt(v.vertexId);

        // Calculate position of the anchor point in the coordinates of our
        // vertex v Try to avoid reprojection so we do not loose precision when
        // map CRS does not match layer CRS
        ...
        if (v.layer->crs() == anchorLayer->crs())
            ...
            anchorPointLayer = toLayerCoordinates(
                v.layer, toMapCoordinates(anchorLayer, anchorPoint));
...
void QgsVertexTool::startDraggingMoveVertex(const QgsPointLocator::Match &m) {
    ...
    setHighlightedVerticesVisible(
        false);  // hide any extra highlight of vertices until we are done with
                 // moving
    ...
    buildExtraVertices(movingVertices, geom.vertexAt(m.vertexIndex()),
                       m.layer());
...
void QgsVertexTool::buildDragBandsForVertices(
    const QSet<Vertex> &movingVertices, const QgsPointXY &dragVertexMapPoint) {
    ...
    for (const Vertex &v : std::as_const(movingVertices)) {
        ...
        if (v0idx != -1 && v1idx != -1 && isCircularVertex(geom, v.vertexId)) {
            ...
            if (!verticesInCircularBands.contains(v)) {
                addDragCircularBand(
                    v.layer, geom.vertexAt(v0idx), pt, geom.vertexAt(v1idx),
            ...
            // skip the rest - no need for further straight of circular bands
            // for this vertex because our circular rubber band spans both
            // towards left and right
            continue;
        }

        if (v0idx != -1) {
            // there is another vertex to the left - let's build a rubber band
            // for it
            ...
            if (isCircularVertex(geom, v0idx)) {
                ...
                if (!verticesInCircularBands.contains(v0)) {
                    addDragCircularBand(
                        v.layer, geom.vertexAt(v0idx - 1), geom.vertexAt(v0idx),
                        pt,
                        movingVertices.contains(
                            Vertex(v.layer, v.fid, v0idx - 1)),
                        movingVertices.contains(Vertex(v.layer, v.fid, v0idx)),
                        true, dragVertexMapPoint);
                    verticesInCircularBands << v0;
                }
            } else {
                // straight segment to the left
                if (!verticesInStraightBands.contains(v0)) {
                    addDragStraightBand(v.layer, geom.vertexAt(v0idx), pt,
                                        movingVertices.contains(v0), true,
                                        dragVertexMapPoint);
        ...
        if (v1idx != -1) {
            // there is another vertex to the right - let's build a rubber band
            // for it
            ...
            if (isCircularVertex(geom, v1idx)) {
                ...
                if (!verticesInCircularBands.contains(v1)) {
                    addDragCircularBand(v.layer, pt, geom.vertexAt(v1idx),
                                        geom.vertexAt(v1idx + 1), true,
                                        movingVertices.contains(v1),
                                        movingVertices.contains(
                                            Vertex(v.layer, v.fid, v1idx + 1)),
                                        dragVertexMapPoint);
                ...
                if (!verticesInStraightBands.contains(v)) {
                    addDragStraightBand(v.layer, pt, geom.vertexAt(v1idx), true,
                                        movingVertices.contains(v1),
                                        dragVertexMapPoint);
...
QList<QgsPointLocator::Match> QgsVertexTool::layerVerticesSnappedToPoint(
    QgsVectorLayer *layer, const QgsPointXY &mapPoint) {
...
QList<QgsPointLocator::Match> QgsVertexTool::layerSegmentsSnappedToSegment(
    QgsVectorLayer *layer, const QgsPointXY &mapPoint1,
    const QgsPointXY &mapPoint2) {
    QList<QgsPointLocator::Match> finalMatches;
    // we want segment matches that have exactly the same vertices as the given
    // segment (mapPoint1, mapPoint2) so rather than doing nearest edge search
    // which could return any segment within a tolerance, we first find matches
    // for one endpoint and then see if there is a matching other endpoint.
    const QList<QgsPointLocator::Match> matches1 =
        layerVerticesSnappedToPoint(layer, mapPoint1);
    for (const QgsPointLocator::Match &m : matches1) {
        ...
        if (v0 != -1 && QgsPointXY(g.vertexAt(v0)) == mapPoint2)
            finalMatches << QgsPointLocator::Match(
                QgsPointLocator::Edge, layer, m.featureId(), 0, m.point(), v0);
        else if (v1 != -1 && QgsPointXY(g.vertexAt(v1)) == mapPoint2)
            finalMatches << QgsPointLocator::Match(QgsPointLocator::Edge, layer,
                                                   m.featureId(), 0, m.point(),
                                                   m.vertexIndex());
    }
    return finalMatches;
}
...
void QgsVertexTool::startDraggingAddVertex(const QgsPointLocator::Match &m) {
    ...
    mDraggingVertex.reset(
        new Vertex(m.layer(), m.featureId(), m.vertexIndex() + 1));
    ...
    if (v0.x() != 0 || v0.y() != 0) addDragBand(map_v0, m.point());
    if (v1.x() != 0 || v1.y() != 0) addDragBand(map_v1, m.point());

    if (QgsProject::instance()->topologicalEditing()) {
        // find other segments coincident with the one user just picked and
        // store them in a list so we can add a new vertex also in those to keep
        // topology correct
        const auto editableLayers = editableVectorLayers();
        for (QgsVectorLayer *vlayer : editableLayers) {
            ...
            const auto snappedSegments =
                layerSegmentsSnappedToSegment(vlayer, pt1, pt2);
            for (const QgsPointLocator::Match &otherMatch : snappedSegments) {
                ...
                // start dragging of snapped point of current layer
                mDraggingExtraSegments
                    << Vertex(otherMatch.layer(), otherMatch.featureId(),
                              otherMatch.vertexIndex());
            }
        }
    }
...
void QgsVertexTool::startDraggingAddVertexAtEndpoint(
    const QgsPointXY &mapPoint) {
    ...
    mDraggingVertex.reset(new Vertex(mMouseAtEndpoint->layer,
                                     mMouseAtEndpoint->fid,
                                     mMouseAtEndpoint->vertexId));
    mDraggingVertexType = AddingEndpoint;
    mDraggingExtraVertices.clear();
    mDraggingExtraVerticesOffset.clear();

    QgsGeometry geom =
        cachedGeometry(mMouseAtEndpoint->layer, mMouseAtEndpoint->fid);
    ...
    QgsPointXY pt0 = geom.vertexAt(
        adjacentVertexIndexToEndpoint(geom, mMouseAtEndpoint->vertexId));
    QgsPointXY pt1 = geom.vertexAt(mMouseAtEndpoint->vertexId);

    cadDockWidget()->setPoints(QList<QgsPointXY>() << pt0 << pt1 << pt1);
}

void QgsVertexTool::startDraggingEdge(const QgsPointLocator::Match &m,
                                      const QgsPointXY &mapPoint) {
    Q_ASSERT(m.hasEdge());
    ...
    // add an extra vertex if it is circular edge - so that we move the whole
    // edge and not just one part of it
    if (isCircularVertex(geom, m.vertexIndex())) {
        movingVertices << Vertex(m.layer(), m.featureId(), m.vertexIndex() - 1);
    } else if (isCircularVertex(geom, m.vertexIndex() + 1)) {
...
QgsPoint QgsVertexTool::matchToLayerPoint(const QgsVectorLayer *destLayer,
                                          const QgsPointXY &mapPoint,
                                          const QgsPointLocator::Match *match) {
    if (match->layer()) {
        switch (match->type()) {
            ...
            case QgsPointLocator::All: {
                ...
                QgsFeature f;
                QgsFeatureIterator fi = match->layer()->getFeatures(
                    QgsFeatureRequest(match->featureId()).setNoAttributes());
                if (fi.nextFeature(f)) {
                    QgsPoint layerPoint =
                        f.geometry().vertexAt(match->vertexIndex());
                    if (match->layer()->crs() == destLayer->crs()) {
                        return layerPoint;
                    } else {
                        QgsCoordinateTransform transform(
                            match->layer()->crs(), destLayer->crs(),
                        ...
                        if (transform.isValid()) {
                            try {
                                ...
                                QgsDebugMsg(
                                    QStringLiteral("transformation to layer "
                                                   "coordinate failed"));
                            }
                        }
                        return layerPoint;
            ...
            case QgsPointLocator::MiddleOfSegment:
                return toLayerCoordinates(
                    destLayer, match->interpolatedPoint(
...
void QgsVertexTool::moveVertex(const QgsPointXY &mapPoint,
    ...
    bool addingVertex = mDraggingVertexType == AddingVertex ||
                        mDraggingVertexType == AddingEndpoint;
    ...
    if (addingVertex && !addingAtEndpoint &&
        QgsProject::instance()->topologicalEditing()) {
        // topo editing: when adding a vertex to an existing segment, there may
        // be other coincident segments that also need adding the same vertex
    ...
    if (QgsProject::instance()->topologicalEditing()) {
        // topo editing: add vertex to existing segments when moving/adding a
        // vertex to such segment. this requires that the snapping match is to a
        // segment and the segment layer's CRS is the same (otherwise we would
        // need to reproject the point and it will not be coincident)
        ...
        for (QgsVectorLayer *layer : editKeys) {
            ...
            for (QgsGeometry g : editGeom) {
                QgsGeometry p = QgsGeometry::fromPointXY(
                    QgsPointXY(layerPoint.x(), layerPoint.y()));
                if (((mapPointMatch->hasEdge() ||
                      mapPointMatch->hasMiddleSegment()) &&
                     mapPointMatch->layer() &&
                     layer->crs() == mapPointMatch->layer()->crs()) ||
                    (mapPointMatch->hasVertex() && !mapPointMatch->layer() &&
                     layer->crs() ==
                         mCanvas->mapSettings()
                             .destinationCrs()))  // also add topological points
                                                  // when snapped on
                                                  // intersection
                {
                    if (g.convertToType(QgsWkbTypes::PointGeometry, true)
                            .contains(p)) {
                        if (!layerPoint.is3D())
                            layerPoint.addZValue(defaultZValue());
                        layer->addTopologicalPoints(layerPoint);
                        if (mapPointMatch->layer())
                            mapPointMatch->layer()->addTopologicalPoints(
                                layerPoint);
                    }
                }
                if (QgsProject::instance()->avoidIntersectionsMode() !=
                    Qgis::AvoidIntersectionsMode::AllowIntersections) {
                    for (QgsAbstractGeometry::vertex_iterator it =
                             g.vertices_begin();
                         it != g.vertices_end(); it++) {
    ...
    setHighlightedVertices(mSelectedVertices);  // update positions of existing
                                                // highlighted vertices
    setHighlightedVerticesVisible(
        true);  // time to show highlighted vertices again

    // restart startDraggingAddVertexAtEndpoint right after it finishes
    if (addingAtEndpoint) {
        if (mMouseAtEndpoint->vertexId != 0) {
            // If we were adding at the end of the feature, we need to update
            // the index
            mMouseAtEndpoint.reset(new Vertex(mMouseAtEndpoint->layer,
                                              mMouseAtEndpoint->fid,
                                              mMouseAtEndpoint->vertexId + 1));
        }
...
void QgsVertexTool::addExtraVerticesToEdits(QgsVertexTool::VertexEdits &edits,
                                            const QgsPointXY &mapPoint,
                                            QgsVectorLayer *dragLayer,
                                            const QgsPoint &layerPoint) {
    Q_ASSERT(mDraggingExtraVertices.count() ==
             mDraggingExtraVerticesOffset.count());
    ...
    for (int i = 0; i < mDraggingExtraVertices.count(); ++i) {
        ...
        if (dragLayer && topo.layer->crs() == dragLayer->crs())
            point = layerPoint;  // this point may come from exact match so it
                                 // may be more precise
        else
...
void QgsVertexTool::addExtraSegmentsToEdits(QgsVertexTool::VertexEdits &edits,
                                            const QgsPointXY &mapPoint,
                                            QgsVectorLayer *dragLayer,
                                            const QgsPoint &layerPoint) {
...
void QgsVertexTool::applyEditsToLayers(QgsVertexTool::VertexEdits &edits) {
    QHash<QgsVectorLayer *, QHash<QgsFeatureId, QgsGeometry>>::iterator it =
        edits.begin();
    for (; it != edits.end(); ++it) {
    ...
    for (it = edits.begin(); it != edits.end(); ++it) {
        ...
        for (; it2 != layerEdits.end(); ++it2) {
            ...
            switch (QgsProject::instance()->avoidIntersectionsMode()) {
                case Qgis::AvoidIntersectionsMode::
            ...
            if (avoidIntersectionsLayers.size() > 0) {
                ...
                int avoidIntersectionsReturn = featGeom.avoidIntersections(
                    avoidIntersectionsLayers, ignoreFeatures);
                switch (avoidIntersectionsReturn) {
                    case 2:
                        emit messageEmitted(
                            tr("The operation would change the geometry type."),
                            Qgis::MessageLevel::Warning);
                        break;

                    case 3:
                        emit messageEmitted(
                            tr("At least one geometry intersected is invalid. "
                               "These geometries must be manually repaired."),
                            Qgis::MessageLevel::Warning);
                        break;
...
void QgsVertexTool::deleteVertex() {
    ...
    if (QgsProject::instance()->topologicalEditing()) {
        // if topo editing is enabled, delete all the vertices that are on the
        // same location
        QSet<Vertex> topoVerticesToDelete;
        for (const Vertex &vertexToDelete : std::as_const(toDelete)) {
            QgsPointXY layerPt = cachedGeometryForVertex(vertexToDelete)
                                     .vertexAt(vertexToDelete.vertexId);
            QgsPointXY mapPt = toMapCoordinates(vertexToDelete.layer, layerPt);
            const auto snappedVertices =
                layerVerticesSnappedToPoint(vertexToDelete.layer, mapPt);
            for (const QgsPointLocator::Match &otherMatch : snappedVertices) {
                Vertex otherVertex(otherMatch.layer(), otherMatch.featureId(),
                                   otherMatch.vertexIndex());
                if (toDelete.contains(otherVertex) ||
                    topoVerticesToDelete.contains(otherVertex))
                    continue;
    ...
    // switch from a plain list to dictionary { layer: { fid: [vertexNr1,
    // vertexNr2, ...] } }
    ...
    // de-duplicate vertices in linear rings - if there is the first vertex
    // selected, then also the last vertex will be selected - but we want just
    // one out of the pair also deselect vertices of parts or rings that will be
    // automatically removed
    QHash<QgsVectorLayer *, QHash<QgsFeatureId, QList<int>>>::iterator lIt =
        toDeleteGrouped.begin();
    for (; lIt != toDeleteGrouped.end(); ++lIt) {
        ...
        for (; fIt != featuresDict.end(); ++fIt) {
            ...
            if (vertexIds.count() >= 2 &&
                ...
                // polygonal rings with less than 4 vertices get deleted
                // automatically linear parts with less than 2 vertices get
                // deleted automatically let's keep that number and don't remove
                // vertices beyond that point
                const int minAllowedVertices =
                    geom.type() == QgsWkbTypes::PolygonGeometry ? 4 : 2;
                for (int i = vertexIds.count() - 1; i >= 0; --i) {
                    QgsVertexId vid;
                    if (geom.vertexIdFromVertexNr(vertexIds[i], vid)) {
                        // also don't try to delete the first vertex of a ring
                        // since we have already deleted the last
                        if (numberOfVertices.at(vid.part).at(vid.ring) <
                                minAllowedVertices ||
                            (0 == vid.vertex &&
    ...
    QHash<QgsVectorLayer *, QHash<QgsFeatureId, QList<int>>>::iterator it =
        toDeleteGrouped.begin();
    for (; it != toDeleteGrouped.end(); ++it) {
        ...
        for (; it2 != featuresDict.end(); ++it2) {
            ...
            for (int vertexId : vertexIds) {
                ...
                if (res != Qgis::VectorEditResult::EmptyGeometry &&
                    res != Qgis::VectorEditResult::Success) {
                    QgsDebugMsg(
                        QStringLiteral("failed to delete vertex %1 %2 %3!")
                            .arg(layer->name())
                            .arg(fid)
                            .arg(vertexId));
                    success = false;
            ...
            if (res == Qgis::VectorEditResult::EmptyGeometry) {
                emit messageEmitted(
                    tr("Geometry has been cleared. Use the add part tool to "
                       "set geometry for this feature."));
            }
        }
    ...
    if (toDelete.count() == 1) {
        ...
        if (geom.vertexAt(vertexId) == QgsPoint()) vertexId -= 1;
    ...
    QgsVertexEditor *editor = vertexEditor();
    if (editor && mLockedFeature) editor->updateEditor(mLockedFeature.get());
}
...
void QgsVertexTool::toggleVertexCurve() {
    ...
    if (mDraggingVertex) {
        if (mDraggingVertexType == AddingVertex ||
            mDraggingVertexType == AddingEndpoint) {
            BcmsApp::instance()->messageBar()->pushMessage(
                tr("Could not convert vertex"),
                tr("Cannot convert vertex before it is added."), Qgis::Warning);
            return;
        }
        stopDragging();
    }
    ...
    if (!QgsWkbTypes::isCurvedType(layer->wkbType())) {
        BcmsApp::instance()->messageBar()->pushMessage(
            tr("Could not convert vertex"),
            tr("Layer of type %1 does not support curved geometries.")
                .arg(QgsWkbTypes::displayString(layer->wkbType())),
            Qgis::Warning);
        return;
    }
    ...
    if (success) {
        ...
        BcmsApp::instance()->messageBar()->pushMessage(
            tr("Could not convert vertex"),
            tr("Start/end of vertices of features and arcs can not be "
               "converted."),
            Qgis::Warning);
    }
...
void QgsVertexTool::setHighlightedVertices(const QList<Vertex> &listVertices,
                                           HighlightMode mode) {
    // we need to make a local copy of vertices - often this method gets called
    // just to refresh positions and so mSelectedVertices is passed. But then in
    // reset mode we clear that array, which could clear also listVertices.
    ...
    if (mode == ModeReset) {
        ...
        // need to clear vertex markers, and rebuild later. We have no way to
        // link a marker to a vertex in order to remove one-by-one
        qDeleteAll(mSelectedVerticesMarkers);
        mSelectedVerticesMarkers.clear();
    }
    ...
    auto createMarkerForVertex = [=](const Vertex &vertex) -> bool {
        ...
        marker->setCenter(
            toMapCoordinates(vertex.layer, geom.vertexAt(vertex.vertexId)));
    ...
    if (mLockedFeature) {
        disconnect(mLockedFeature.get(), &QgsLockedFeature::selectionChanged,
                   this, &QgsVertexTool::lockedFeatureSelectionChanged);

        mLockedFeature->deselectAllVertices();
        for (const Vertex &vertex : std::as_const(mSelectedVertices)) {
            // we should never be able to select vertices that are not from the
            // locked feature cppcheck-suppress assertWithSideEffect
            Q_ASSERT(mLockedFeature->featureId() == vertex.fid &&
                     mLockedFeature->layer() == vertex.layer);
...
void QgsVertexTool::highlightAdjacentVertex(double offset) {
    if (mSelectedVertices.isEmpty()) return;
...
void QgsVertexTool::initSelectionRubberBand() {
    if (!mSelectionRubberBand) {
        mSelectionRubberBand = std::make_unique<QgsRubberBand>(
            mCanvas, QgsWkbTypes::PolygonGeometry);
...
bool QgsVertexTool::matchEdgeCenterTest(const QgsPointLocator::Match &m,
                                        const QgsPointXY &mapPoint,
                                        QgsPointXY *edgeCenterPtr) {
    QgsPointXY p0, p1;
    m.edgePoints(p0, p1);

    QgsGeometry geom = cachedGeometry(m.layer(), m.featureId());
    if (isCircularVertex(geom, m.vertexIndex()) ||
        isCircularVertex(geom, m.vertexIndex() + 1))
    ...
    if (!visible_extent.contains(p0) || !visible_extent.contains(p1)) {
        // clip line segment to the extent so the mid-point marker is always
        // visible
        QgsGeometry extentGeom = QgsGeometry::fromRect(visible_extent);
        QgsGeometry lineGeom =
            QgsGeometry::fromPolylineXY(QgsPolylineXY() << p0 << p1);
        lineGeom = extentGeom.intersection(lineGeom);
        // The intersection can be empty if the whole lineGeom is just outside
        // the extent
        if (!lineGeom.isEmpty()) {
            QgsPolylineXY polyline = lineGeom.asPolyline();
            Q_ASSERT_X(polyline.count() == 2,
                       "QgsVertexTool::matchEdgeCenterTest",
                       QgsLineString(polyline).asWkt().toUtf8().constData());
            p0 = polyline[0];
            p1 = polyline[1];
        }
    }
    ...
    if (edgeCenterPtr) *edgeCenterPtr = edgeCenter;
...
void QgsVertexTool::CircularBand::updateRubberBand(const QgsPointXY &mapPoint) {
    ...
    QgsGeometryUtils::segmentizeArc(QgsPoint(v0), QgsPoint(v1), QgsPoint(v2),
                                    points);
    // it would be useful to have QgsRubberBand::setPoints() call
    band->reset();
    for (const QgsPoint &p : std::as_const(points)) band->addPoint(p);
}

void QgsVertexTool::validationErrorFound(const QgsGeometry::Error &e) {
    QgsGeometryValidator *validator =
        qobject_cast<QgsGeometryValidator *>(sender());
    if (!validator) return;
...
void QgsVertexTool::validationFinished() {
    ...
    for (; it != mValidations.end(); ++it) {
        GeometryValidation &validation = *it;
        if (validation.validator == validator) {
            // QgsStatusBar *sb = BcmsApp::instance()->statusBarIface();
            // sb->showMessage( tr( "Validation finished (%n error(s) found).",
            // "number of geometry errors", validation.errorMarkers.size() ) );
...
void QgsVertexTool::GeometryValidation::start(QgsGeometry &geom,
                                              QgsVertexTool *t,
                                              QgsVectorLayer *l) {
    tool = t;
    layer = l;
    Qgis::GeometryValidationEngine method =
        Qgis::GeometryValidationEngine::QgisInternal;
    if (QgsSettingsRegistryCore::settingsDigitizingValidateGeometries.value() ==
        2)
        method = Qgis::GeometryValidationEngine::Geos;

    validator = new QgsGeometryValidator(geom, nullptr, method);
    connect(validator, &QgsGeometryValidator::errorFound, tool,
            &QgsVertexTool::validationErrorFound);
    connect(validator, &QThread::finished, tool,
            &QgsVertexTool::validationFinished);
    validator->start();
}
...
void QgsVertexTool::GeometryValidation::addError(QgsGeometry::Error e) {
    ...
    if (e.hasWhere()) {
        ...
        marker->setCenter(tool->canvas()->mapSettings().layerToMapCoordinates(
            layer, e.where()));
        marker->setIconType(QgsVertexMarker::ICON_X);
    ...
    auto *mb = BcmsApp::instance()->messageBar();
    mb->pushMessage(e.what(), tr(""), Qgis::MessageLevel::Warning);
}
...
void QgsVertexTool::validateGeometry(QgsVectorLayer *layer,
...
void QgsVertexTool::zoomToVertex(const Vertex &vertex) {
    QgsPointXY newCenter =
        cachedGeometryForVertex(vertex).vertexAt(vertex.vertexId);
    QgsPointXY mapPoint =
        mCanvas->mapSettings().layerToMapCoordinates(vertex.layer, newCenter);
...
QList<Vertex> QgsVertexTool::verticesInRange(QgsVectorLayer *layer,
                                             QgsFeatureId fid, int vertexId0,
                                             int vertexId1, bool longWay) {
    QgsGeometry geom = cachedGeometry(layer, fid);

    if (vertexId0 > vertexId1) std::swap(vertexId0, vertexId1);
    ...
    if (geom.vertexAt(startVertexIndex) == geom.vertexAt(endVertexIndex)) {
        // closed curve - we need to find out which way around the curve is
        // shorter
        ...
        for (int i = startVertexIndex + 1; i <= endVertexIndex; ++i) {
            ...
            lengthTotal += len;
            if (i > vertexId0 && i <= vertexId1) length0to1 += len;
            ptOld = pt;
        }

        bool use0to1 = length0to1 < lengthTotal / 2;
        if (longWay) use0to1 = !use0to1;
...
void QgsVertexTool::rangeMethodReleaseEvent(QgsMapMouseEvent *e) {
    if (e->button() == Qt::RightButton) {
        ...
        if (mRangeSelectionFirstVertex) {
            // pick final vertex, make selection and switch back to normal
            // selection
            QgsPointLocator::Match m = snapToEditableLayer(e);
            if (m.hasVertex()) {
                if (m.layer() == mRangeSelectionFirstVertex->layer &&
                    m.featureId() == mRangeSelectionFirstVertex->fid) {
                    QList<Vertex> lst = verticesInRange(
                        m.layer(), m.featureId(),
                        mRangeSelectionFirstVertex->vertexId, m.vertexIndex(),
                        e->modifiers() & Qt::ControlModifier);
...
void QgsVertexTool::rangeMethodMoveEvent(QgsMapMouseEvent *e) {
    if (e->buttons()) return;  // only with no buttons pressed
    ...
    if (!m.hasVertex()) {
        QList<Vertex> lst;
        if (mRangeSelectionFirstVertex) lst << *mRangeSelectionFirstVertex;
        setHighlightedVertices(lst);
        return;
    }
    ...
    if (mRangeSelectionFirstVertex) {
        ...
        if (m.layer() == mRangeSelectionFirstVertex->layer &&
            m.featureId() == mRangeSelectionFirstVertex->fid) {
            QList<Vertex> lst = verticesInRange(
                m.layer(), m.featureId(), mRangeSelectionFirstVertex->vertexId,
                m.vertexIndex(), e->modifiers() & Qt::ControlModifier);
            setHighlightedVertices(lst);
        }
    }
}
