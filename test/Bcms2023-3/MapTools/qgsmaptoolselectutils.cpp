...
QgsMapLayer *QgsMapToolSelectUtils::getCurrentTargetLayer(
    QgsMapCanvas *canvas) {
    ...
    if (!layer) {
        BcmsApp::instance()->messageBar()->pushMessage(
            ...
            QObject::tr("To select features, choose a vector layer in the "
                        "layers panel"),
...
void QgsMapToolSelectUtils::setRubberBand(QgsMapCanvas *canvas,
    ...
    QgsPointXY ll =
        transform->toMapCoordinates(selectRect.left(), selectRect.bottom());
    QgsPointXY lr =
        transform->toMapCoordinates(selectRect.right(), selectRect.bottom());
    QgsPointXY ul =
        transform->toMapCoordinates(selectRect.left(), selectRect.top());
    QgsPointXY ur =
        transform->toMapCoordinates(selectRect.right(), selectRect.top());
...
QgsRectangle QgsMapToolSelectUtils::expandSelectRectangle(QgsPointXY mapPoint,
        ...
        switch (layer->type()) {
            case QgsMapLayerType::VectorLayer: {
                ...
                if (vLayer->geometryType() != QgsWkbTypes::PolygonGeometry) {
                    // if point or line use an artificial bounding box of 10x10
                    // pixels to aid the user to click on a feature accurately
                    boxSize = 5;
                } else {
            ...
            case QgsMapLayerType::VectorTileLayer:
                // mixed layer type, so aim for somewhere between the vector
                // layer polygon/point sizes
                boxSize = 2;
                break;
    ...
    QgsPointXY ll =
        transform->toMapCoordinates(static_cast<int>(point.x() - boxSize),
                                    static_cast<int>(point.y() + boxSize));
    QgsPointXY ur =
        transform->toMapCoordinates(static_cast<int>(point.x() + boxSize),
                                    static_cast<int>(point.y() - boxSize));
...
void QgsMapToolSelectUtils::selectMultipleFeatures(
    QgsMapCanvas *canvas, const QgsGeometry &selectGeometry,
...
bool transformSelectGeometry(const QgsGeometry &selectGeometry,
    ...
    try {
        if (!ct.isShortCircuited() &&
            selectGeomTrans.type() == QgsWkbTypes::PolygonGeometry) {
...
void QgsMapToolSelectUtils::selectSingleFeature(
    QgsMapCanvas *canvas, const QgsGeometry &selectGeometry,
    Qt::KeyboardModifiers modifiers) {
    ...
    switch (layer->type()) {
        case QgsMapLayerType::VectorLayer: {
            ...
            QgsFeatureIds selectedFeatures =
                getMatchingFeatures(canvas, selectGeometry, false, true);
            if (selectedFeatures.isEmpty()) {
                if (!(modifiers & Qt::ShiftModifier ||
                      modifiers & Qt::ControlModifier)) {
                    // if no modifiers then clicking outside features clears the
                    // selection but if there's a shift or ctrl modifier, then
                    // it's likely the user was trying to modify an existing
                    // selection by adding or subtracting features and just
                    // missed the feature
            ...
            // either shift or control modifier switches to "toggle" selection
            // mode
            if (modifiers & Qt::ShiftModifier ||
                modifiers & Qt::ControlModifier) {
                QgsFeatureId selectId = *selectedFeatures.constBegin();
                QgsFeatureIds layerSelectedFeatures =
                    vlayer->selectedFeatureIds();
        ...
        case QgsMapLayerType::VectorTileLayer: {
            QgsVectorTileLayer *vtLayer =
                qobject_cast<QgsVectorTileLayer *>(layer);

            QgsCoordinateTransform ct(canvas->mapSettings().destinationCrs(),
            ...
            QgsRenderContext renderContext =
                QgsRenderContext::fromMapSettings(canvas->mapSettings());
            QgsExpressionContext expressionContext =
                canvas->createExpressionContext();
            expressionContext << QgsExpressionContextUtils::layerScope(vtLayer);
            renderContext.setExpressionContext(expressionContext);

            vtLayer->selectByGeometry(
                selectGeomTrans, context, behavior,
                Qgis::SelectGeometryRelationship::Intersect, flags,
                &renderContext);
            break;
        }
...
void QgsMapToolSelectUtils::setSelectedFeatures(
    QgsMapCanvas *canvas, const QgsGeometry &selectGeometry,
    ...
    switch (layer->type()) {
        case QgsMapLayerType::VectorLayer: {
            QgsVectorLayer *vLayer = qobject_cast<QgsVectorLayer *>(layer);
            QgsFeatureIds selectedFeatures = getMatchingFeatures(
                canvas, selectGeometry, doContains, singleSelect);
        ...
        case QgsMapLayerType::VectorTileLayer: {
            ...
            vtLayer->selectByGeometry(
                selectGeomTrans, context, selectBehavior,
                doContains ? Qgis::SelectGeometryRelationship::Within
                           : Qgis::SelectGeometryRelationship::Intersect,
                Qgis::SelectionFlags(), &renderContext);
            break;
        }
...
QgsFeatureIds QgsMapToolSelectUtils::getMatchingFeatures(
    QgsMapCanvas *canvas, const QgsGeometry &selectGeometry, bool doContains,
    bool singleSelect) {
    ...
    QgsMapLayer *targetLayer =
        QgsMapToolSelectUtils::getCurrentTargetLayer(canvas);
    QgsVectorLayer *vlayer = qobject_cast<QgsVectorLayer *>(targetLayer);
    if (!vlayer) return newSelectedFeatures;
    ...
    if (!selectGeomTrans.isGeosValid()) {
        ...
        if (selectGeomTrans.isEmpty()) return newSelectedFeatures;
    }

    std::unique_ptr<QgsGeometryEngine> selectionGeometryEngine(
        QgsGeometry::createGeometryEngine(selectGeomTrans.constGet()));
    ...
    const QString canvasFilter =
        QgsMapCanvasUtils::filterForLayer(canvas, vlayer);
    if (canvasFilter == QLatin1String("FALSE")) return newSelectedFeatures;
    ...
    if (!canvasFilter.isEmpty()) request.setFilterExpression(canvasFilter);
    if (r) {
    ...
    while (fit.nextFeature(f)) {
        ...
        if (r && !r->willRenderFeature(f, context)) continue;

        QgsGeometry g = f.geometry();
        QString errorMessage;
        if (doContains) {
            // if we get an error from the contains check then it indicates that
            // the geometry is invalid and GEOS choked on it. in this case we
            // consider the bounding box intersection check which has already
            // been performed by the iterator as sufficient and allow the
            // feature to be selected
            const bool notContained =
                !selectionGeometryEngine->contains(g.constGet(),
                                                   &errorMessage) &&
                (errorMessage.isEmpty() || /* message will be non empty if
                                              geometry g is invalid */
                 !selectionGeometryEngine->contains(
                     g.makeValid().constGet(),
                     &errorMessage)); /* second chance for invalid geometries,
                                         repair and re-test */

            if (!errorMessage.isEmpty()) {
                // contains relation test still failed, even after trying to
                // make valid!
                QgsMessageLog::logMessage(
                    QObject::tr("Error determining selection: %1")
                        .arg(errorMessage),
            ...
            // if we get an error from the intersects check then it indicates
            // that the geometry is invalid and GEOS choked on it. in this case
            // we consider the bounding box intersection check which has already
            // been performed by the iterator as sufficient and allow the
            // feature to be selected
            const bool notIntersects =
                !selectionGeometryEngine->intersects(g.constGet(),
                                                     &errorMessage) &&
                (errorMessage.isEmpty() || /* message will be non empty if
                                              geometry g is invalid */
                 !selectionGeometryEngine->intersects(
                     g.makeValid().constGet(),
                     &errorMessage)); /* second chance for invalid geometries,
                                         repair and re-test */

            if (!errorMessage.isEmpty()) {
                // intersects relation test still failed, even after trying to
                // make valid!
                QgsMessageLog::logMessage(
                    QObject::tr("Error determining selection: %1")
                        .arg(errorMessage),
    ...
    QgsDebugMsgLevel("Number of new selected features: " +
                         QString::number(newSelectedFeatures.size()),
                     2);
...
QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::QgsMapToolSelectMenuActions(
    QgsMapCanvas *canvas, QgsVectorLayer *vectorLayer,
    ...
    connect(mVectorLayer, &QgsMapLayer::destroyed, this,
            &QgsMapToolSelectMenuActions::onLayerDestroyed);

    mFutureWatcher = new QFutureWatcher<QgsFeatureIds>(this);
    connect(mFutureWatcher, &QFutureWatcher<void>::finished, this,
            &QgsMapToolSelectMenuActions::onSearchFinished);
}

QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::
    ~QgsMapToolSelectMenuActions() {
    removeHighlight();
    if (mJobData) mJobData->isCanceled = true;
    if (mFutureWatcher) mFutureWatcher->waitForFinished();
}

void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::populateMenu(
    QMenu *menu) {
    mActionChooseAll = new QAction(textForChooseAll(), this);
    menu->addAction(mActionChooseAll);
    connect(mActionChooseAll, &QAction::triggered, this,
            &QgsMapToolSelectMenuActions::chooseAllCandidateFeature);
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::startFeatureSearch() {
    const QString canvasFilter =
        QgsMapCanvasUtils::filterForLayer(mCanvas, mVectorLayer);
    if (canvasFilter == QLatin1String("FALSE")) return;
    ...
    mJobData->ct = QgsCoordinateTransform(
        mCanvas->mapSettings().destinationCrs(), mVectorLayer->crs(),
        mJobData->context.transformContext());
    ...
    mJobData->context.expressionContext()
...
QgsFeatureIds QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::search(
    std::shared_ptr<DataForSearchingJob> data) {
    QgsFeatureIds newSelectedFeatures;
    ...
    if (r) {
        request.setSubsetOfAttributes(r->usedAttributes(data->context),
                                      data->source->fields());
    ...
    while (fit.nextFeature(f) && !data->isCanceled) {
        ...
        // make sure to only use features that are visible
        if (r && !r->willRenderFeature(f, data->context)) continue;

        QgsGeometry g = f.geometry();
        QString errorMessage;

        // if we get an error from the intersects check then it indicates that
        // the geometry is invalid and GEOS choked on it. in this case we
        // consider the bounding box intersection check which has already been
        // performed by the iterator as sufficient and allow the feature to be
        // selected
        const bool notIntersects =
            !selectionGeometryEngine->intersects(g.constGet(), &errorMessage) &&
            (errorMessage.isEmpty() || /* message will be non empty if geometry
                                          g is invalid */
             !selectionGeometryEngine->intersects(
                 g.makeValid().constGet(),
                 &errorMessage)); /* second chance for invalid geometries,
        ...
        if (!errorMessage.isEmpty()) {
            // intersects relation test still failed, even after trying to make
            // valid!
            QgsMessageLog::logMessage(
                QObject::tr("Error determining selection: %1")
                    .arg(errorMessage),
    ...
    if (r) r->stopRender(data->context);
    return filterIds(newSelectedFeatures, data->existingSelection,
                     data->selectBehavior);
}

void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::onSearchFinished() {
    if (mFutureWatcher && !mFutureWatcher->isFinished()) return;
    ...
    if (!mAllFeatureIds.isEmpty())
        connect(mActionChooseAll, &QAction::hovered, this,
                &QgsMapToolSelectMenuActions::highlightAllFeatures);
    else
        mActionChooseAll->setEnabled(false);
    if (mAllFeatureIds.count() > 1) populateChooseOneMenu(mAllFeatureIds);
}

QString QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::textForChooseAll(
    qint64 featureCount) const {
    ...
    switch (mBehavior) {
        ...
        case Qgis::SelectBehavior::IntersectSelection:
            return tr("Intersect All with Selection (%1)")
                .arg(featureCountText);
        case Qgis::SelectBehavior::RemoveFromSelection:
...
QString
QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::textForChooseOneMenu()
    const {
    switch (mBehavior) {
        case Qgis::SelectBehavior::SetSelection:
            return tr("Select Feature");
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::populateChooseOneMenu(
    ...
    while (displayedFeatureIds.count() <= 20 &&
           it != ids.constEnd())  // for now hardcoded, but maybe define a
                                  // settings for this
        displayedFeatureIds.insert(*(it++));

    QgsExpressionContext context(
        QgsExpressionContextUtils::globalProjectLayerScopes(mVectorLayer));
    QgsExpression exp = mVectorLayer->displayExpression();
    exp.prepare(&context);

    QgsFeatureRequest request =
        QgsFeatureRequest().setFilterFids(displayedFeatureIds);
    QgsFeature feat;
    ...
    while (featureIt.nextFeature(feat)) {
        ...
        connect(featureAction, &QAction::triggered, this,
                [this, id]() { chooseOneCandidateFeature(id); });
        connect(featureAction, &QAction::hovered, this,
                [this, id]() { this->highlightOneFeature(id); });
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::
    chooseOneCandidateFeature(QgsFeatureId id) {
    if (!mVectorLayer) return;
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::
    chooseAllCandidateFeature() {
    if (!mFutureWatcher) return;
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::
    ...
    if (!mAllFeatureIds.empty()) {
        ...
        for (const QgsFeatureId &id : std::as_const(mAllFeatureIds)) {
            ...
            if (count > 1000)  // for now hardcoded, but maybe define a settings
                               // for this
                return;
        }
    }
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::highlightOneFeature(
    QgsFeatureId id) {
    removeHighlight();
...
void QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::styleHighlight(
...
QgsFeatureIds QgsMapToolSelectUtils::QgsMapToolSelectMenuActions::filterIds(
    const QgsFeatureIds &ids, const QgsFeatureIds &existingSelection,
