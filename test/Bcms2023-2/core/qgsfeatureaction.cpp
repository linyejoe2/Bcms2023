...
#include "mainwindow.h"
// TODO 暫時不用 #include "qgsidentifyresultsdialog.h"
...
QgsAttributeDialog *QgsFeatureAction::newDialog(bool cloneFeature) {
    ...
    QgsAttributeEditorContext context(
        MainWindow::instance()->createAttributeEditorContext());
...
//        MainWindow::instance()->findChild<QgsAttributeDialog *>(name);
//    if (dialog) {
//        delete h;
//        dialog->raise();
...
QgsFeatureAction::AddFeatureResult QgsFeatureAction::addFeature(
    const QgsAttributeMap &defaultAttributes, bool showModal,
    std::unique_ptr<QgsExpressionContextScope> scope, bool hideParent,
    ...
    if (scope) context.appendScope(scope.release());
    ...
    *mFeature = newFeature;
    ...
    // override application-wide setting if layer is non-spatial -- BECAUSE it's
    // bad UX if it appears that nothing happens when you click the add row
    // button for a non-spatial layer. Unlike spatial layers, where you can SEE
    // the newly created spatial object on the map, creating a new feature in a
    ...
    // finally, if this action has specifically forced suppression of the form,
    ...
    if (isDisabledAttributeValuesDlg) {
        ...
        if (highlight) dialog->setHighlight(highlight.release());

        connect(dialog->attributeForm(), &QgsAttributeForm::featureSaved, this,
                &QgsFeatureAction::onFeatureSaved);

        if (!showModal) {
            setParent(dialog);  // keep dialog until the dialog is closed and
                                // destructed
    ...
    return mFeatureSaved ? AddFeatureResult::Success
                         : (dialogWasShown ? AddFeatureResult::Canceled
                                           : AddFeatureResult::FeatureError);
}
...
void QgsFeatureAction::hideParentWidget() {
    ...
    if (dialog) {
        ...
        if (triggerWidget && triggerWidget->window()->objectName() !=
